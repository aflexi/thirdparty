...
class IXR_Message {
    ...
    function parse() {
        ...
        // [ija 20100613] preg_replace depends on pcre, which has a 100k character limit default in php.ini. S
        // Since we have big result files sometimes - we only compare first 30 characters for xml declaraction.
        // Original: $this->message = preg_replace('/<\?xml(.*)?\?'.'>/', '', $this->message);
        $message = preg_replace('/<\?xml(.*)?\?'.'>/', '', substr($this->message, 0, 128));
        // And replace back:
        $this->message = substr_replace($this->message, $message, 0, 128);
        // DONE
...
class IXR_Client {
    ...
    function query() {
        ...
        while (!feof($fp)) {
            ...
            if (!$gettingHeaders) {
                // NOTE [yclian 20100621] Bug addressed in Wordpress #12559. This was causing line-breaks returned from GM got trimmed.
                // See: http://core.trac.wordpress.org/ticket/12559
                // $contents .= trim($line);
                $contents .= $line;
            }
        }
