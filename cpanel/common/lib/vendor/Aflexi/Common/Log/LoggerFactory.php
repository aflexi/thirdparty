...
require_once dirname(__FILE__).'/LoggerSimple.php';
...
class Aflexi_Common_Log_LoggerFactory{
    ...
    /**
     * Name of default logger class name. Must be an instance of 
     * Aflexi_Common_Log_Logger.
     * 
     * @var string
     */
     ...
     * @return Aflexi_Common_Log_Logger
     */
    public static function getLogger($name = NULL, $className = NULL, $options = array()){
        ...
        if(empty($className)){
            if(!empty(self::$loggerClassName)){
                ...
                if(class_exists('Aflexi_Web_Log_SfLoggerAdapter') && sfContext::hasInstance()){
                    ...
                    $className = 'Aflexi_Common_Log_LoggerSimple';
                }
            }
        }

        $class = new ReflectionClass($className);
        $rt = $class->newInstance($name);

        if($rt instanceof Aflexi_Common_Object_Configurable){
            $rt->configure($options);
        }
        ...
        self::$loggers[$name] = $rt;
        return $rt;
    }

    /**
     * Set the default Aflexi_Common_Log_Logger class, used if unspecified in {@code #getLogger()}.
     */
